...
(function (a, b) {
    ...
    function a(b, c) {
        this.scrollDom = document.getElementById(b);
        this.options = c || {};
        ...
        setTimeout(function () {
            if (d.optDown.auto) {
                if (d.optDown.autoShowLoading) {
                    d.triggerDownScroll()
                } else {
                    d.optDown.callback && d.optDown.callback(d)
                }
            }
            d.optUp.auto && d.triggerUpScroll()
        }, 30)
    }

    a.prototype.extendDownScroll = function (c) {
        ...
        a.extend(c, {
            ...
            inited: function (e, d) {
                e.downTipDom = d.getElementsByClassName("downwarp-tip")[0];
                e.downProgressDom = d.getElementsByClassName("downwarp-progress")[0]
            },
            inOffset: function (d) {
                if (d.downTipDom) {
                    d.downTipDom.innerHTML = "下拉刷新"
                }
                if (d.downProgressDom) {
                    d.downProgressDom.classList.remove("mescroll-rotate")
                }
            },
            outOffset: function (d) {
                if (d.downTipDom) {
                    d.downTipDom.innerHTML = "释放更新"
                }
            },
            onMoving: function (e, g, d) {
                if (e.downProgressDom) {
                    var f = 360 * g;
                    //e.downProgressDom.style.webkitTransform = "rotate(" + f + "deg)";下拉刷新
                    //e.downProgressDom.style.transform = "rotate(" + f + "deg)"
                }
            },
            beforeLoading: function (e, d) {
                return false
            },
            showLoading: function (d) {
                if (d.downTipDom) {
                    d.downTipDom.innerHTML = "加载中 ..."
                }
                if (d.downProgressDom) {
                    //d.downProgressDom.classList.add("mescroll-rotate")弹回去
                }
            },
            callback: function (d) {
                d.resetUpScroll()
    ...
    a.prototype.extendUpScroll = function (b) {
        ...
        a.extend(b, {
            ...
            htmlLoading: '<p class="upwarp-progress"></p><p class="upwarp-tip">加载中..</p>',
    ...
    a.prototype.initDownScroll = function () {
        var d = this;
        d.optDown = d.options.down || {};
        if (d.optDown.use == false) {
            return
        }
        d.extendDownScroll(d.optDown);
        d.downwarp = document.createElement("div");
        d.downwarp.className = d.optDown.warpClass;
        d.downwarp.innerHTML = '<div class="downwarp-content">' + d.optDown.htmlContent + "</div>";
        d.scrollDom.insertBefore(d.downwarp, d.scrollDom.firstChild);
        d.scrollDom.addEventListener("touchstart", c);
        d.scrollDom.addEventListener("mousedown", c);

        function c(f) {
            if (d.isScrollTo) {
                f.preventDefault()
            }
            d.startTop = d.scrollDom.scrollTop;
            if (d.optDown.mustToTop) {
                d.startY = f.touches ? f.touches[0].pageY : f.clientY
            }
        }

        d.scrollDom.addEventListener("touchmove", b);
        d.scrollDom.addEventListener("mousemove", b);

        function b(k) {
            if (d.startTop != null && d.scrollDom.scrollTop <= 0 && !d.isDownScrolling && (!d.isUpScrolling || (d.isUpScrolling && d.optUp.isBoth)) && !d.optDown.isLock) {
                if (d.optDown.mustToTop && d.startTop > 0) {
                    return
                }
                var g = k.touches ? k.touches[0].pageX : k.clientX;
                var f = k.touches ? k.touches[0].pageY : k.clientY;
                if (!d.preX) {
                    d.preX = g
                }
                if (!d.preY) {
                ...
                var n = Math.abs(d.preX - g);
                var m = Math.abs(d.preY - f);
                var l = Math.sqrt(n * n + m * m);
                var o = f - d.preY;
                d.preX = g;
                d.preY = f;
                if (l != 0) {
                    var i = Math.asin(m / l) / Math.PI * 180;
                    if (i < d.optDown.minAngle) {
                        return
                    }
                }
                if (!d.startY && !d.optDown.mustToTop) {
                    d.startY = f
                }
                var h = f - d.startY;
                if (h > 0) {
                    ...
                    if (d.downHight < d.optDown.offset) {
                        if (d.movetype != 1) {
                            d.movetype = 1;
                            d.optDown.inOffset(d);
                            d.downwarp.classList.remove(d.optDown.resetClass);
                            d.scrollDom.classList.add(d.optDown.hardwareClass);
                        ...
                        if (o > 0) {
                            d.downHight += o * d.optDown.outOffsetRate
                        } else {
                            d.downHight += o
                        }
                    }
                    d.downwarp.style.height = d.downHight + "px";
                    var j = d.downHight / d.optDown.offset;
                    d.optDown.onMoving(d, j, d.downHight)
                }
            }
        }

        d.scrollDom.addEventListener("touchend", e);
        d.scrollDom.addEventListener("mouseup", e);
        d.scrollDom.addEventListener("mouseleave", e);
        ...
        function e(f) {
            if (d.isMoveDown) {
                if (d.downHight >= d.optDown.offset) {
                    ...
                    d.downwarp.classList.add(d.optDown.resetClass);
                    d.downHight = 0;
                    d.downwarp.style.height = 0
                }
                d.scrollDom.style.webkitOverflowScrolling = "touch";
                d.scrollDom.classList.remove(d.optDown.hardwareClass);
                d.movetype = 0;
                d.isMoveDown = false
            }
            d.startY = 0;
    ...
    a.prototype.initUpScroll = function () {
        ...
        b.scrollDom.appendChild(b.upwarp);
        b.scrollDom.addEventListener("scroll", function () {
            var d = b.scrollDom.scrollTop;
            if (!b.isUpScrolling && (!b.isDownScrolling || (b.isDownScrolling && b.optDown.isBoth))) {
                if (!b.optUp.isLock) {
                    var c = b.scrollDom.scrollHeight - b.scrollDom.clientHeight - d;
                    if (c <= b.optUp.offset) {
                        b.triggerUpScroll()
                    }
                }
                if (b.optUp.toTop.src) {
                    if (d >= b.optUp.toTop.offset) {
                        b.showTopBtn()
                    } else {
                        b.hideTopBtn()
                    }
            ...
            b.optUp.onScroll && b.optUp.onScroll(b, d)
        });
        setTimeout(function () {
    ...
    a.prototype.endSuccess = function (c, e) {
    ...
    a.prototype.loadFull = function () {
        ...
        if (b.optUp.loadFull.use && !b.optUp.isLock && b.scrollDom.scrollHeight <= b.scrollDom.clientHeight) {
            setTimeout(function () {
                if (b.scrollDom.scrollHeight <= b.scrollDom.clientHeight) {
                    b.triggerUpScroll()
                }
    ...
    a.prototype.showEmpty = function () {
        ...
        if (g) {
            ...
            var f = "";
            if (d.icon) {
                f += '<p class="dingIcon"><img  src="' + d.icon + '"/></p>'
            }
            if (d.tip) {
                f += '<p class="dingTit">' + d.tip + "</p>"
            }
            if (d.btntext) {
                f += '<input type="button" class="gobuy" id="" value="' + d.btntext + '" />'
            }
    ...
    a.prototype.removeEmpty = function () {
        if (this.emptyDom) {
            var b = this.emptyDom.parentNode;
            if (b) {
                b.removeChild(this.emptyDom)
            }
            this.emptyDom = null
        }
    };
    a.prototype.showTopBtn = function () {
        if (!this.topBtnShow) {
            ...
            if (b.toTopBtn == null) {
                ...
                document.body.appendChild(b.toTopBtn)
            }
    ...
    a.prototype.scrollTo = function (j, l) {
        l = l || 300;
        var g = 20;
        var f = l / g;
        var h = this;
        var b = h.scrollDom.scrollHeight - h.scrollDom.clientHeight;
        if (j < 0) {
            j = 0
        }
        if (j > b) {
        ...
        var e = window.setInterval(function () {
            if (d < f) {
                if (d == f - 1) {
                    h.scrollDom.scrollTop = j
                } else {
                    h.scrollDom.scrollTop -= c
                }
                d++
            } else {
                h.isScrollTo = false;
